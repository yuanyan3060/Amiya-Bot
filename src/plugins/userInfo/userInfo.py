...
from ..util.image import create_image

Pool = require('gacha').Pool
info_cmd = on_command(
    cmd="个人信息",
    rule=regex("^(信赖|关系|好感|我的信息|个人信息)$"),
    priority=5,
    block=True
)
...
async def handle_info(bot: Bot, event: MessageEvent, state: T_State):
    user, _ = User.get_or_create(user_id=event.user_id)
    feeling = user.user_feeling if user.user_feeling <= 4000 else 4000
    pool, _ = Pool.get_or_create(pool_id=user.gacha_pool)
    ...
    await info_cmd.finish(MessageSegment.image(create_image(text=text)))
