...
class DownLoader:
    ...
    async def get_all_pic(self, force_update=False):
        ...
        if cur_version == version and not force_update:
            return
        raw_glob_data = await request_string(f"{self.pics_source}/file_dict.json")
        glob_data = json.loads(raw_glob_data)

        for type_, file_list in glob_data.items():
            file_list = [i for i in file_list if i.rfind("#") == -1]
            dirs = pathConfig.images / type_
            dirs.mkdir(parents=True, exist_ok=True)
            local_file_list = [i.name for i in dirs.iterdir()]
            count = len(local_file_list)
            logger.info('{} {}/{}', type_, count, len(file_list))
            tasks = []
            for file in file_list:
                if file not in local_file_list:
                    tasks.append(self.get_pic(type_, file))
            if len(tasks) == 0:
                continue
        ...
        async with aiofiles.open(version_file_path, "w", encoding="UTF-8") as fp:
            await fp.write(cur_version)
